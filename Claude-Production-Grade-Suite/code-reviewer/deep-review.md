# SkyClaw Deep Code Quality Review
...
- **Severity:** High
- **File:** `crates/skyclaw-channels/src/cli.rs:89`
...
- **Severity:** High
- **File:** `crates/skyclaw-agent/src/runtime.rs` (absent), `crates/skyclaw-vault/src/detector.rs`
...
- **Severity:** Medium
- **File:** `crates/skyclaw-gateway/src/server.rs:8`, `crates/skyclaw-gateway/src/router.rs:4`
- **Description:** `skyclaw-gateway` imports `skyclaw_agent::AgentRuntime` directly. This creates a cross-impl-crate dependency (gateway -> agent -> core), violating ADR-002's "impl crates depend on core only" rule.
- **Impact:** Gateway cannot be compiled independently of agent. Changes to agent internals can break gateway.
- **Recommendation:** Define an agent trait in `skyclaw-core` and use trait objects in gateway.
- **Status vs F-02:** Still valid, unchanged.

#### DF-04: Vault URI resolution not wired into config/provider path (CONFIRMED from F-16)
- **Severity:** Medium
- **File:** `crates/skyclaw-vault/src/resolver.rs`, `crates/skyclaw-providers/src/lib.rs:34-37`
- **Description:** `resolve()`, `parse_vault_uri()`, and `is_vault_uri()` are all implemented in `skyclaw-vault`. However, `create_provider()` in `skyclaw-providers/src/lib.rs:34` uses `config.api_key.clone()` directly without checking for `vault://` URIs. A user setting `api_key = "vault://skyclaw/anthropic_key"` would send the literal URI as their API key.
...
- **Severity:** Low
- **File:** `crates/skyclaw-memory/src/sqlite.rs:228-246`, `crates/skyclaw-memory/src/markdown.rs:300-319`
...
- **Recommendation:** Either add `Display`/`FromStr` impls on `MemoryEntryType` in `skyclaw-core`, or extract to a shared module in `skyclaw-memory`.
- **Status vs F-09:** Still valid, unchanged.

#### DF-06: Duplicated HTTP error-handling blocks in providers
- **Severity:** Medium
- **File:** `crates/skyclaw-providers/src/anthropic.rs:266-281,324-339`, `crates/skyclaw-providers/src/openai_compat.rs:326-341,413-427`
...
- **Recommendation:** Extract a helper function like `fn check_response_status(status: StatusCode, body: &str, provider_name: &str) -> Result<(), SkyclawError>` in a shared utility module or in each provider file.

#### DF-07: Regex recompiled on every call in `expand_env_vars`
- **Severity:** Low
- **File:** `crates/skyclaw-core/src/config/env.rs:5`
...
- **Severity:** Low
- **File:** `crates/skyclaw-providers/src/lib.rs:7`
- **Description:** `#![allow(dead_code)]` suppresses all dead-code warnings for the entire `skyclaw-providers` crate. The SSE response types (e.g., `AnthropicSseMessageStart`, `OpenAIStreamToolCall`) are legitimately only used in deserialization, but a blanket suppression hides any future dead code.
...
- **Severity:** Low
- **File:** `crates/skyclaw-vault/src/local.rs:202-207`
...
- **Severity:** Info
- **File:** `crates/skyclaw-channels/src/lib.rs:32`
...
- **Severity:** Low
- **File:** `crates/skyclaw-core/src/config/env.rs:19`, `crates/skyclaw-core/src/config/loader.rs:148`
...
- **Severity:** Medium
- **File:** `crates/skyclaw-gateway/src/session.rs:12`
...
- **Severity:** Medium
- **File:** `crates/skyclaw-memory/src/markdown.rs:182-192`
...
- **Severity:** Low
- **File:** `crates/skyclaw-agent/src/context.rs:90-91`
...
- **Recommendation:** Pass `max_tokens` and `temperature` from `SkyclawConfig` through `AgentRuntime` to `build_context`. Define constants if they must be hardcoded.
...
- **Severity:** Medium
- **File:** `crates/skyclaw-providers/src/openai_compat.rs:237-253`
...
- **Severity:** Medium
- **File:** `crates/skyclaw-channels/src/telegram.rs:67-69,288-297`, `crates/skyclaw-test-utils/src/lib.rs:289-293`
...
- **Severity:** Low
- **File:** `crates/skyclaw-core/src/types/config.rs` (multiple fields)
- **Description:** Several configuration fields use `String` where enums would provide type safety:
  - `SkyclawSection.mode: String` (should be `enum Mode { Cloud, Local, Auto }`)
...
#### DF-18: `SkyclawConfig` re-exports not flat (CONFIRMED from F-13)
- **Severity:** Low
- **File:** `crates/skyclaw-core/src/types/mod.rs`, `crates/skyclaw-core/src/lib.rs`
- **Description:** `lib.rs` does `pub use types::*` which re-exports sub-modules (`message`, `file`, `config`, `session`, `error`) but not their individual types. Consuming crates use inconsistent import paths: `skyclaw_core::error::SkyclawError` vs `skyclaw_core::types::error::SkyclawError`.
- **Impact:** Inconsistent imports across crates; new contributors must discover the correct import path by trial.
- **Recommendation:** Add `pub use error::SkyclawError;`, `pub use message::*;`, etc. in `types/mod.rs` to flatten the namespace.
- **Status vs F-13:** Still valid. The inconsistency is visible: `skyclaw-memory/src/lib.rs:17` uses `skyclaw_core::error::SkyclawError` while `skyclaw-agent/src/runtime.rs:7` uses `skyclaw_core::types::error::SkyclawError`.
...
- **Severity:** Info
- **File:** `crates/skyclaw-core/src/types/error.rs:44-45`
- **Description:** `#[from] serde_json::Error` allows any JSON error to auto-convert to `SkyclawError::Serialization`. In practice, the provider code correctly uses `.map_err()` for provider-specific JSON errors (e.g., `anthropic.rs:286`), so this is well-managed.
...
- **Severity:** Info
- **File:** `crates/skyclaw-vault/src/detector.rs:121,135,136`
